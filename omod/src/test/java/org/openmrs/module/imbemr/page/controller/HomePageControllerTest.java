...
public class HomePageControllerTest {
    ...
    /**
     * @see org.openmrs.module.imbemr.page.controller.HomePageController#controller(org.openmrs.ui.framework.page.PageModel model,
    ...
    public void controller_shouldLimitWhichAppsAreShownOnTheHomePageBasedOnLocation() throws Exception {
        ...
        Extension extension = new Extension("ext1", "app1", ImbEmrConstants.HOME_PAGE_EXTENSION_POINT_ID, "link", "label", "url", 1);
        ...
        extension = new Extension("ext2", "app1", ImbEmrConstants.HOME_PAGE_EXTENSION_POINT_ID, "link", "label", "url", 2);
        extension.setRequire("sessionContext.get(\"sessionLocation\").hasTag(\"tag2\")");
        when(appFrameworkConfig.isEnabled(extension)).thenReturn(Boolean.TRUE);
        extensions.add(extension);

        extension = new Extension("ext3", "app1", ImbEmrConstants.HOME_PAGE_EXTENSION_POINT_ID, "link", "label", "url", 3);
        extension.setRequire("sessionContext.get(\"sessionLocation\").hasTag(\"tag3\")");
        when(appFrameworkConfig.isEnabled(extension)).thenReturn(Boolean.TRUE);
        extensions.add(extension);

        extension = new Extension("ext4", "app1", ImbEmrConstants.HOME_PAGE_EXTENSION_POINT_ID, "link", "label", "url", 4);
        extension.setRequire("sessionContext.get(\"sessionLocation\").getUuid()==\"" + locationUuid + "\"");
        when(appFrameworkConfig.isEnabled(extension)).thenReturn(Boolean.TRUE);
        extensions.add(extension);

        extension = new Extension("ext5", "app1", ImbEmrConstants.HOME_PAGE_EXTENSION_POINT_ID, "link", "label", "url", 5);
